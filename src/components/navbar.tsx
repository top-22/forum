  ...
  return (
    <Navbar bg="primary" className="d-flex flex-column vh-100">
      ...
      <Nav className="flex-column d-flex flex-column flex-grow-1">
        ...
        <Nav.Link onClick={() => router.push('/')} className="mx-auto">Home</Nav.Link>
        <hr className="mx-auto w-75" /> {/* Divider after Home */}
        <div className="flex-grow-1">
          <Nav.Link onClick={() => router.push('room/1')} className="mx-auto">Raum 1</Nav.Link>
          <Nav.Link onClick={() => router.push('room/2')} className="mx-auto">Raum 2</Nav.Link>
        </div>
        <hr className="mx-auto w-75" /> {/* Divider before Settings */}
        <Nav.Link onClick={() => setShowSettings(true)} className="mx-auto">Settings</Nav.Link>
      </Nav>
    </Navbar>
  );
};
